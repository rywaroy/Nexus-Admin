...
import type { CaptchaVerifyPassingData, SliderCaptchaActionType } from "@vben/common-ui";
  ...
  <Page description="用于前端简单的拖动校验场景" title="滑块校验">
    <Card class="mb-5" title="基础示例">
      <div class="flex-center p-4 px-[30%]">
        ...
        <Button class="ml-2" type="primary" @click="handleBtnClick(el1)"> 还原 </Button>
      </div>
    </Card>
    <Card class="mb-5" title="自定义圆角">
      <div class="flex-center p-4 px-[30%]">
        <SliderCaptcha ref="el2" class="rounded-full" @success="handleSuccess" />
        <Button class="ml-2" type="primary" @click="handleBtnClick(el2)"> 还原 </Button>
      </div>
    </Card>
    <Card class="mb-5" title="自定义背景色">
      <div class="flex-center p-4 px-[30%]">
        ...
        />
        <Button class="ml-2" type="primary" @click="handleBtnClick(el3)"> 还原 </Button>
      </div>
    </Card>
    <Card class="mb-5" title="自定义拖拽图标">
      <div class="flex-center p-4 px-[30%]">
        ...
        <Button class="ml-2" type="primary" @click="handleBtnClick(el4)"> 还原 </Button>
      </div>
    </Card>
    <Card class="mb-5" title="自定义文本">
      <div class="flex-center p-4 px-[30%]">
        <SliderCaptcha ref="el5" success-text="成功" text="拖动" @success="handleSuccess" />
        <Button class="ml-2" type="primary" @click="handleBtnClick(el5)"> 还原 </Button>
      </div>
    </Card>
    <Card class="mb-5" title="自定义内容(slot)">
      <div class="flex-center p-4 px-[30%]">
        ...
        <Button class="ml-2" type="primary" @click="handleBtnClick(el6)"> 还原 </Button>
      </div>
    </Card>
  </Page>
</template>
