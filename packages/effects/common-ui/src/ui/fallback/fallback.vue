...
<template>
  <div class="flex-col-center size-full duration-300">
    ...
    <div class="flex-col-center">
      ...
      <p v-else-if="titleText" class="mt-8 text-2xl text-foreground md:text-3xl lg:text-4xl">
        {{ titleText }}
      </p>
      <slot v-if="$slots.describe" name="describe"></slot>
      <p v-else-if="descText" class="md:text-md my-4 text-muted-foreground lg:text-lg">
        {{ descText }}
      </p>
